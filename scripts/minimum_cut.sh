...
do
  ...
  do
    ...
    ${exec_dir}/min_cut ${datasets_dir}/kron_connected/${stream_name} $workers $readers no $eps_val false ${num_batch_table[$stream_name]}
  done
done
...
do
  ...
  do
    echo -n "$stream_name, ${num_batch_table[$stream_name]}, ${eps_val}, " >> ${out_file}
    ${exec_dir}/min_cut ${datasets_dir}/ktree/${stream_name} $workers $readers no $eps_val false ${num_batch_table[$stream_name]}
  done
done
...
do
  ...
  do
    echo -n "$stream_name, ${num_batch_table[$stream_name]}, ${eps_val}, " >> ${out_file}
    ${exec_dir}/min_cut ${datasets_dir}/real_world/${stream_name} $workers $readers no $eps_val false ${num_batch_table[$stream_name]}
  done
done
...
do
  ...
  do
    echo -n "$stream_name, ${num_batch_table[$stream_name]}, ${eps_val}, " >> ${out_file}
    ${exec_dir}/min_cut ${datasets_dir}/reddit/${stream_name} $workers $readers no $eps_val false ${num_batch_table[$stream_name]}
  done
done

mv ${out_file} $result_dir/mc.csv
