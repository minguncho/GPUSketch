...
do
  ...
  do
    for (( it=1; it <= $num_it; ++it ))
    do
        echo -n "$stream_name, CPU (Adj. List), ${k}, " >> ${out_file}
        ${exec_dir}/st_cpu_adjlist ${datasets_dir}/kron/${stream_name} $workers $readers ${k}
    done
  done
done
...
do
  ...
  do
    ...
    do
        ...
        ${exec_dir}/st_cpu_gpu_transfer ${datasets_dir}/kron/${stream_name} $workers $readers ${k} true
    done
  done
done
  ...
  do
    ...
    do
        echo -n "$stream_name, GPU Kernel, ${k}, " >> ${out_file}
         ${exec_dir}/st_gpu_sketch_update ${datasets_dir}/kron/${stream_name} $workers $readers ${k} true
    done
  done
done

mv ${out_file} $result_dir/sys_standalone_hybrid.csv