...
int main(int argc, char **argv) {
  ...
  std::vector<double> rounds_factor;
  ...
  for (size_t i = 0; i < rounds_required.size(); i++) {
    rounds_factor.push_back((double)i / default_samples);
  }
  ...
  std::vector<double> sketch_sizes = {0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6};
  std::vector<double> num_samples(sketch_sizes.size());

  for (size_t i = 0; i < rounds_required.size(); i++) {
    if (rounds_required[i] == 0) continue;

    double num_factor = rounds_factor[i];

    for (size_t j = 0; j < sketch_sizes.size() - 1; j++) {
      if ((num_factor > sketch_sizes[j]) && (num_factor <= sketch_sizes[j + 1])) {
        num_samples[j] += rounds_required[i];
      }
    }
  }
  std::ofstream out("runtime_results.csv", std::ios_base::out | std::ios_base::app);
  for (size_t i = 0; i < sketch_sizes.size(); i++) {
    out << sketch_sizes[i] << "," << num_samples[i] << std::endl;
  }
}